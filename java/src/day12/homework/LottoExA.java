...
public class LottoExA {
	...
	public static void main(String[] args) {
		...
		printResult(count, Array.contains1(user, bonus, user.length));
		
		sc.close();
	}
	
	/**로또 번호와 사용자 번호가 주어지면 몇개가 같은지 알려주는 메소드
	...
	public static int count(int lotto[], int user[]) {
		...
		int count = 0;
		for(int tmp : lotto) {
			if(Array.contains1(user, tmp, user.length)) {
				count++;
			}
		}
		return count;
	}
	/**일치하는 개수와 보너스번화 사용자 번호가 일치하는 여부를 알려주면
	 * 등수를 출력하는 메소드
